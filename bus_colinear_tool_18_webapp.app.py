...
SUBWAY_DB = {
    ...
    "6号线": "金安桥,苹果园,杨庄,西黄村,廖公庄,田村,海淀五路居,慈寿寺,花园桥,白石桥南,二里沟,车公庄西,車公庄,平安里,北海北,南锣鼓巷,东四,朝阳门,东大桥,呼家楼,金台路,十里堡,青年路,褡袢坡,黄渠,常营,草房,物资学院路,通州北关,北运河西,北运河东,郝家府,东夏园,潞城",
    ...
    "8号线": "朱辛庄,育知路,平西府,回龙观东大街,霍营,育新,西小口,永泰庄,林萃桥,森林公园南门,奥利匹克公园,奥体中心,北土城,安华桥,安德里北街,鼓楼大街,什刹海,南锣鼓巷,中国美术馆,金鱼胡同,王府井,前门,珠市口,天桥,永定门外,木樨园,海户屯,大红门,大红门南,和义,东高地,火箭万源,五福堂,德茂,瀛海",
...
def draw_gantt(station_data, report):
    ...
    for s in station_data:
        # 如果站名包含换行符 "\n"，只截取第一行作为图表标签
        clean_name = s['name'].split('\n')[0]
        clean_names.append(clean_name)
    
    ax.set_xticks(range(len(station_data)))
    ax.set_xticklabels(clean_names, rotation=90, fontproperties=my_font, fontsize=11)
...
# --- 4. Streamlit 界面主体 ---
st.set_page_config(page_title="公交共线分析工具 V2.3", layout="wide")
st.title("🚇 公交与轨道共线分析系统 V2.3")
...
if mode == "既有全线分析":
    ...
    input_str = st.sidebar.text_area("输入站点名称(英文逗号分隔)", 
                                    value="马连洼, 上地软件园, 东北旺",
                                    help="注意：必须使用英文半角逗号 ',' 隔开站点名称")
else:
    st.sidebar.markdown("🔗 [高德坐标拾取工具](https://lbs.amap.com/tools/picker)")
    input_str = st.sidebar.text_area("输入“名称 坐标”(空格+英文分号分隔)", 
                                    value="点位A 116.27,40.03; 点位B 116.30,40.04",
                                    help="格式示例：站名(空格)116.xx,39.xx(英文分号);")
    is_coord_mode = True
...
with st.sidebar.expander("📖 网页使用说明书"):
    st.markdown("""
    ### ⚠️ 核心输入规范
    1. **英文标点**：所有逗号 `,` 和分号 `;` 必须使用英文输入法下的半角符号。
    2. **坐标格式**：站名与坐标之间必须有**一个空格**。
    ...
    **3. 规划线路(坐标)**
    - 点击上方链接前往拾取器。
    - 格式：`站名(空格)116.xx,39.xx;`。
    - 每个站点后必须跟**英文分号**。

    **4. 导出数据**
    - 点击下方按钮即可下载结构化的 CSV 报告。
    """)
